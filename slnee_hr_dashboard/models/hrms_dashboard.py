...
class Employee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        if employee:
            ...
            leaves_alloc_req = self.env['hr.holidays'].sudo().search_count([
                ('state', 'in', ['confirm', 'validate1']),
                ('type', '=', 'add'),
            ])
            #
            ...
            #
            # loans_count = self.env['hr.loan'].sudo().search_count([])
            # employee[0].update({
            #     'loans_count': loans_count,
            # })
            #
            expenses_count = self.env['hr.expense'].sudo().search_count([])
            employee[0].update({
            ...
            if employee[0]['birthday']:
                ...
                age = False
            if employee[0]['date_of_join']:
                diff = relativedelta(datetime.today(), datetime.strptime(employee[0]['date_of_join'], '%Y-%m-%d'))
                years = diff.years
                months = diff.months
                days = diff.days
                experience = _('{} years {} months {} days').format(years, months, days)
            else:
                experience = False
            if employee:
                data = {
    ...
    def join_resign_trends(self):
        ...
        cr.execute('''select to_char(date_of_join, 'Month YYYY') as l_month, count(id) from hr_employee 
        WHERE date_of_join BETWEEN CURRENT_DATE - INTERVAL '12 months'
        AND CURRENT_DATE + interval '1 month - 1 day'
        group by l_month;''')
        date_of_join = cr.fetchall()
    ...
    #         where resign_date> date '%s' or resign_date is null and date_of_join < date '%s'
